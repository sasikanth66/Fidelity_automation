#!/usr/bin/env python3
"""Sell to close call options from current holdings."""
import argparse
import json
import os
...
parser.add_argument("--index", type=int, default=None, help="Sell a specific call position by index (0-based)")
args = parser.parse_args()

with open(HOLDINGS_PATH) as f:
    holdings = json.load(f)

if not holdings["calls"]:
    print("No call positions to sell.")
    exit(0)

# Show positions
print("Current call positions:")
for i, pos in enumerate(holdings["calls"]):
    print(f"  [{i}] {pos['quantity']}x {pos['ticker']} {pos['expiration']} ${pos['strike']} call in {pos['account']}")
print()

# Determine which to sell
if args.index is not None:
    if args.index >= len(holdings["calls"]):
        print(f"ERROR: Index {args.index} out of range (0-{len(holdings['calls'])-1})")
        exit(1)
    positions_to_sell = [args.index]
else:
    positions_to_sell = list(range(len(holdings["calls"])))
...
for idx in positions_to_sell:
    pos = holdings["calls"][idx]
    print(f"[{mode}] SELL TO CLOSE {pos['quantity']}x {pos['ticker']} {pos['expiration']} ${pos['strike']} call in {pos['account']}")
