#!/usr/bin/env python3
"""Buy to open a put option using config with dynamic strike/expiration."""
import argparse
import json
import os
...
print(f"Finding nearest OTM put strike...")
strike = find_nearest_otm_strike(ticker, expiration, "put", current_price)
print(f"Strike: ${strike:.2f}")

dry = not args.confirm
mode = "DRY RUN" if dry else "LIVE"
print(f"\n[{mode}] BUY TO OPEN {quantity}x {ticker} {expiration} ${strike} put in {account}")
if limit_price:
