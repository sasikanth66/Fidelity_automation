...
Sell to close put option positions from current holdings. Uses bid price.
...
```bash
# Dry run — sell all put positions
tmux send-keys -t <pane_id> "python /Users/sasi/stock_sr_scanner/fidelity_scripts/sell_put.py" Enter

# Dry run — sell a specific position by index
tmux send-keys -t <pane_id> "python /Users/sasi/stock_sr_scanner/fidelity_scripts/sell_put.py --index 0" Enter

# Execute for real
tmux send-keys -t <pane_id> "python /Users/sasi/stock_sr_scanner/fidelity_scripts/sell_put.py --confirm" Enter
```
